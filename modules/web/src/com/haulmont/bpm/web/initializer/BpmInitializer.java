...
public class BpmInitializer {
    ...
    @EventListener
    public void initializeHttpFilter(ServletContextInitializedEvent event) {
        boolean isUberJar = Boolean.parseBoolean(AppContext.getProperty("cuba.uberJar"));
        if (isUberJar) {
            Filter filter = servletRegistrationManager.createFilter(event.getApplicationContext(), BpmFormContentFilter.class.getName());
            event.getSource().addFilter("bpmFormContentFilter", filter)
                    .addMappingForUrlPatterns(
                            EnumSet.of(DispatcherType.REQUEST, DispatcherType.ASYNC), true, "/dispatch/modeler/*");
        }
    }
}
